...
#define PATHLMAX 5    /* max free path on graph */
...
/* WORKS FOR ELLIPSE, HAS TO BE ADAPTED TO GENERAL BILLIARD */
{
    ...
    if (anglemin <= 0.0)
        anglemin = PI / ((double)NPART);
    if (anglemax >= PI)
        anglemax = PI * (1.0 - 1.0 / ((double)NPART));
...
/* draw the particles */
{
    ...
    for (i = 0; i < nparticles; i++)
    {
        ...
        if ((TEST_ACTIVE) && (active[i]))
            active[i] = xy_in_billiard(x1, y1);
    ...
    for (j = 0; j < time; j++)
    {
        for (i = 0; i < nparticles; i++)
        {
            if (configs[i][2] < 0.0)
            {
                ...
                if ((c >= 0) && (circles[c].color == 0))
                    nobst++;
                circles[c].color++;

                /* take care of circles doubled because of periodic boundary conditions */
                if ((circles[c].active) && (B_DOMAIN == D_CIRCLES_IN_TORUS) && (circles[c].partner != c))
                    circles[circles[c].partner].color++;
                ...
                if (ncol > 1) /* disregard very first collision */
                {
                    if (B_DOMAIN != D_CIRCLES_IN_TORUS)
                    {
                        ...
                        if (k < NPATHBINS)
                            npath[k]++;
                        if (total_pathlength > max_free_path)
                            max_free_path = total_pathlength;
...
void print_particle_numbers(double *configs[NPARTMAX])
{
    ...
    if (first) /* compute box limits */
    {
        ...
        for (i = 0; i < ncircles; i++)
            if ((circles[i].active) && (circles[i].xc - circles[i].radius < xleft))
                xleft = circles[i].xc - circles[i].radius;
        for (i = 0; i < ncircles; i++)
            if ((circles[i].active) && (circles[i].xc + circles[i].radius > xright))
                xright = circles[i].xc + circles[i].radius;
...
void draw_statistics()
{
    ...
    for (i = 0; i < ncircles; i++)
        if ((circles[i].active) && (!circles[i].double_circle))
        {
            n = circles[i].color;
            if (n < colmax)
                pegcollisions[n]++;
...
void animation(int circle_config)
{
    ...
    /* init circle configuration if the domain is D_CIRCLES */
    if ((B_DOMAIN == D_CIRCLES) || (B_DOMAIN == D_CIRCLES_IN_RECT) || (B_DOMAIN == D_CIRCLES_IN_GENUSN) || (B_DOMAIN == D_CIRCLES_IN_TORUS))
        init_circles_pinball(circle_config, circles);
    ...
    if ((B_DOMAIN == D_CIRCLES_IN_RECT) || (B_DOMAIN == D_CIRCLES_IN_GENUSN))
        ...
        for (i = 0; i < ncircles; i++)
        {
            if (vabs(circles[i].xc) + circles[i].radius > 0.99)
                circles[i].active = 0;
            if (vabs(circles[i].xc) + circles[i].radius > 0.99 * LAMBDA)
                circles[i].active = 0;
        }
    else if (B_DOMAIN == D_CIRCLES_IN_TORUS)
        for (i = 0; i < ncircles; i++)
        {
            if (vabs(circles[i].yc) - circles[i].radius > 1.0)
                circles[i].active = 0;
            if (vabs(circles[i].xc) - circles[i].radius > LAMBDA)
                circles[i].active = 0;
        }
    ...
    if (DRAW_BILLIARD)
        draw_billiard();
    if (ERASE_OUTSIDE)
        erase_rectangle_outside(270.0, 0.1, 0.15);
    ...
    // for (i=0; i<=INT32_MAX-2; i++)
    {
        ...
        if (SHOWTRAILS)
            draw_config_showtrails(newcolor, configs, active);
        else
        ...
        for (j = 0; j < ncircles; j++)
            if (circles[j].color > nmaxpeg)
                nmaxpeg = circles[j].color;
