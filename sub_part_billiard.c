...
#define BOUNDARY_SHIFT 100000.0 /* shift of boundary parameterization for circles in domain */
...
void paint_billiard_interior() /* paints billiard interior, for use before draw_conf */
{
    ...
    case (D_REULEAUX):
    {
        ...
        if (LAMBDA > 0.0)
            x2 = cos(omega * 0.5) + sqrt(LAMBDA * LAMBDA - sin(omega * 0.5) * sin(omega * 0.5));
        else
            x2 = cos(omega * 0.5) - sqrt(LAMBDA * LAMBDA - sin(omega * 0.5) * sin(omega * 0.5));
    ...
    case (D_ALT_REU):
    {
        ...
        for (i = 0; i <= NPOLY; i++)
        {
            for (j = 0; j < NSEG; j++)
            {
                s = 2.0 * (((double)j / (double)NSEG) - 0.5) * beta2;
                if (i % 2 == 0)
                    x1 = x2plus - LAMBDA * cos(s);
                else
                    x1 = x2minus + LAMBDA * cos(s);
                y1 = LAMBDA * sin(s);
                angle = i * omega + APOLY * PID;
                x = cos(angle) * x1 - sin(angle) * y1;
                y = sin(angle) * x1 + cos(angle) * y1;
            ...
            if (circles[k].active)
            {
                ...
                else
                {
                    if (circles[k].new >= 1)
                    {
                        ...
                        rgb_color_scheme(circles[k].color, rgb1);
...
/* determine initial configuration for start at point pos = (x,y) */
{
    ...
    if (s0 < 0.0)
    {
        ...
        {
            config[0] = x1 + LAMBDA;
            if ((x1 <= -LAMBDA + margin) || (x1 >= LAMBDA - margin))
                config[1] = alpha + PI; /* corners */
    ...
    if (intb && (c0 > 0.0))
    {
        ...
        if ((y1 >= -1.0) && (y1 <= 1.0))
        {
            ...
            if ((y1 <= -1.0 + margin) || (y1 >= 1.0 - margin))
                config[1] = alpha + PI; /* corners */
    ...
    if (intb)
    {
        ...
        if (intc)
            for (i = 0; i < 2; i++)
            {
                ...
                {
                    if (l > 0.0)
                        config[0] = 2.0 * LAMBDA + PI + psi;
                    else
                        config[0] = psi - 3.0 * asin(-l) + PI;
...
/* conf[0] is arclength on boundary */
{
    ...
    if (LAMBDA > 0.0)
        x2 = cos(omega2) + sqrt(LAMBDA * LAMBDA - sin(omega2) * sin(omega2));
    else
        x2 = cos(omega2) - sqrt(LAMBDA * LAMBDA - sin(omega2) * sin(omega2));
...
/* determine initial configuration for start at point pos = (x,y) */
{
    ...
    if (intb)
        x1 = 1.0e12; /* to make inactive particles too close to a corner */
...
/* determine initial configuration for start at point pos = (x,y) */
{
    ...
    if (nt > 0) /* there is at least one intersection */
    {
        ...
        while (phiint[ntmin] < 0.0)
            phiint[ntmin] += DPI;
        while (phiint[ntmin] >= DPI)
            phiint[ntmin] -= DPI;
        ...
        if (circles[i].active)
        {
            ...
            if (delta > margin) /* there is an intersection with circle i */
            {
                ...
                if (t > margin)
                {
                    ...
                    if ((vabs(xint[nt]) < LAMBDA) && (vabs(yint[nt]) < 1.0))
                        nt++;
                }
            }
        }
        ...
        if (circles[i].active)
        {
            ...
            if (delta > margin) /* there is an intersection with circle i */
            {
                ...
                if (t > margin)
                {
                    ...
                    if (in_polygon(xint[nt], yint[nt], 1.0, NPOLY, APOLY))
                        nt++;
                }
            }
        }
...

{
    ...
    else /* there is no intersection with the circles - compute intersection with boundary */
    {
        ...
        if (config[0] < 2.0 * LAMBDA)
            config[0] = 4.0 * LAMBDA + 2.0 - config[0];
        else if (config[0] < 2.0 * LAMBDA + 2.0)
            config[0] = 6.0 * LAMBDA + 4.0 - config[0];
        else if (config[0] < 4.0 * LAMBDA + 2.0)
            config[0] = 4.0 * LAMBDA + 2.0 - config[0];
        else
            config[0] = 6.0 * LAMBDA + 4.0 - config[0];
    ...
    for (i = 0; i < nsides; i++)
    {
        ...
        if (vabs(delta) > margin) /* there is an intersection with the line containing segment i */
        {
            ...
            if (t > margin)
            {
                ...
                if (vabs(dx) > margin)
                    s = (xi - polyline[i].x1) / dx;
                else
                    s = (yi - polyline[i].y1) / (polyline[i].y2 - polyline[i].y1);
                //                     printf("s = %.2f\n", s);
...
/* returns 1 if (x,y) represents a point in the billiard */
{
    ...
    case D_STADIUM:
    {
        if ((x > -0.5 * LAMBDA) && (x < 0.5 * LAMBDA) && (y > -1.0) && (y < 1.0))
            return (1);
        else if (module2(x + 0.5 * LAMBDA, y) < 1.0)
            return (1);
        else if (module2(x - 0.5 * LAMBDA, y) < 1.0)
            return (1);
        else
    ...
    case D_DIAMOND:
    {
        ...
        if ((x * x + y * y < 1.0) && ((x - LAMBDA) * (x - LAMBDA) + (y - LAMBDA) * (y - LAMBDA) > r2) && ((x - LAMBDA) * (x - LAMBDA) + (y + LAMBDA) * (y + LAMBDA) > r2) && ((x + LAMBDA) * (x + LAMBDA) + (y - LAMBDA) * (y - LAMBDA) > r2) && ((x + LAMBDA) * (x + LAMBDA) + (y + LAMBDA) * (y + LAMBDA) > r2))
            return (1);
        else
            return (0);
        break;
    ...
    case D_TRIANGLE:
    {
        if ((x > -LAMBDA) && (y > -1.0) && (LAMBDA * y + x < 0.0))
            return (1);
        else
            return (0);
        break;
    ...
    case D_REULEAUX:
    {
        ...
        so = sin(omega2);
        if (LAMBDA > 0.0)
            x2 = co + sqrt(LAMBDA * LAMBDA - so * so);
        else
            x2 = co - sqrt(LAMBDA * LAMBDA - so * so);
        ...
        for (k = 0; k < NPOLY; k++)
        {
            ...
            if (LAMBDA > 0.0)
                condition = condition * ((x1 - x2) * (x1 - x2) + y1 * y1 > LAMBDA * LAMBDA);
            else
                condition = condition * ((x1 - x2) * (x1 - x2) + y1 * y1 < LAMBDA * LAMBDA);
        }
        return (condition);
    ...
    case D_ALT_REU:
    {
        ...
        for (k = 0; k < NPOLY; k++)
        {
            ...
            y1 = -x * sin(angle) + y * cos(angle);
            if (k % 2 == 0)
                condition = condition * ((x1 - x2plus) * (x1 - x2plus) + y1 * y1 > LAMBDA * LAMBDA);
            else
                condition = condition * ((x1 - x2minus) * (x1 - x2minus) + y1 * y1 < LAMBDA * LAMBDA);
        }
        return (condition);
        break;
    }
    case D_ANGLE:
    {
        if (y <= 0.0)
            return (0);
        else if (x * sin(LAMBDA * PI) < -y * cos(LAMBDA * PI))
            return (1);
        else
            return (0);
        break;
    ...
    case D_PENROSE:
    {
        ...
        else if ((vabs(y) >= MU) && (x * x / (LAMBDA * LAMBDA) + (y1 - MU) * (y1 - MU) / ((1.0 - MU) * (1.0 - MU)) >= 1.0))
            return (0);
        /* small ellipses */
        else if ((vabs(x) <= c) && (4.0 * (x1 - c) * (x1 - c) / (MU * MU * PENROSE_RATIO * PENROSE_RATIO) + y * y / (MU * MU) <= 1.0))
            return (0);
        /* straight parts */
        else if ((vabs(x) >= c) && (vabs(y) <= width))
            return (0);
        else
            return (1);
    }
    case D_CIRCLES:
    {
        ...
        for (k = 0; k < ncircles; k++)
            if (circles[k].active)
                condition = condition * out_circle(x - circles[k].xc, y - circles[k].yc, circles[k].radius);
...
void init_circles(t_circle circles[NMAXCIRCLES])
{
    ...
    case (C_GOLDEN_SPIRAL):
    {
        ...
        for (i = 0; i < ncircles; i++)
        {
            ...
            if ((circles[i].yc < YMAX + MU) && (circles[i].yc > YMIN - MU))
                circles[i].active = 1;
        }
        break;
    }
...
/* add a rectangle to polyline pattern */
{
    ...
    if (nsides + 4 < NMAXPOLY)
        for (i = nsides; i < nsides + 4; i++)
        {
            polyline[i].color = 0;
            if (i < nsides + 3)
                polyline[i].x2 = polyline[i + 1].x1;
            else
                polyline[i].x2 = polyline[nsides].x1;
            if (i < nsides + 3)
                polyline[i].y2 = polyline[i + 1].y1;
            else
                polyline[i].y2 = polyline[nsides].y1;
        }
    else
        printf("Increase NMAXPOLY\n");

    if (nsides + 4 < NMAXCIRCLES)
        for (i = nsides; i < nsides + 4; i++)
        {
...
/* compute reflection of point z wrt axis through z1 and z2 */
{
    ...
    r = module2(u.x1, u.y1);
    if (r == 0)
        return (0); /* z1 and z2 are the same */
...
void init_polyline(t_segment polyline[NMAXPOLY], t_circle circles[NMAXCIRCLES])
{
    ...
    case (P_TOKARSKY):
    {
        ...
        polyline[0].x1 = 0.0;
        polyline[0].y1 = 2.0;
        polyline[0].angle = 0.0;
        polyline[1].x1 = 1.0;
        polyline[1].y1 = 2.0;
        polyline[1].angle = -PID;
        polyline[2].x1 = 1.0;
        polyline[2].y1 = 1.0;
        polyline[2].angle = 0.0;
        polyline[3].x1 = 2.0;
        polyline[3].y1 = 1.0;
        polyline[3].angle = -PID;
        polyline[4].x1 = 2.0;
        polyline[4].y1 = 0.0;
        polyline[4].angle = 0.5 * PID;
        polyline[5].x1 = 3.0;
        polyline[5].y1 = 1.0;
        polyline[5].angle = 0.0;
        polyline[6].x1 = 4.0;
        polyline[6].y1 = 1.0;
        polyline[6].angle = -PID;
        polyline[7].x1 = 4.0;
        polyline[7].y1 = 0.0;
        polyline[7].angle = 0.5 * PID;
        polyline[8].x1 = 5.0;
        polyline[8].y1 = 1.0;
        polyline[8].angle = 0.0;
        polyline[9].x1 = 6.0;
        polyline[9].y1 = 1.0;
        polyline[9].angle = -PID;
        polyline[10].x1 = 6.0;
        polyline[10].y1 = 0.0;
        polyline[10].angle = 0.5 * PID;
        polyline[11].x1 = 7.0;
        polyline[11].y1 = 1.0;
        polyline[11].angle = PID;
        polyline[12].x1 = 7.0;
        polyline[12].y1 = 2.0;
        polyline[12].angle = 0.0;
        polyline[13].x1 = 8.0;
        polyline[13].y1 = 2.0;
        polyline[13].angle = PID;
        polyline[14].x1 = 8.0;
        polyline[14].y1 = 3.0;
        polyline[14].angle = PI;
        polyline[15].x1 = 7.0;
        polyline[15].y1 = 3.0;
        polyline[15].angle = 1.5 * PID;
        polyline[16].x1 = 6.0;
        polyline[16].y1 = 4.0;
        polyline[16].angle = -PID;
        polyline[17].x1 = 6.0;
        polyline[17].y1 = 3.0;
        polyline[17].angle = PI;
        polyline[18].x1 = 5.0;
        polyline[18].y1 = 3.0;
        polyline[18].angle = -PID;
        polyline[19].x1 = 5.0;
        polyline[19].y1 = 2.0;
        polyline[19].angle = PI;
        polyline[20].x1 = 3.0;
        polyline[20].y1 = 2.0;
        polyline[20].angle = PID;
        polyline[21].x1 = 3.0;
        polyline[21].y1 = 3.0;
        polyline[21].angle = PI;
        polyline[22].x1 = 2.0;
        polyline[22].y1 = 3.0;
        polyline[22].angle = PID;
        polyline[23].x1 = 2.0;
        polyline[23].y1 = 4.0;
        polyline[23].angle = PI;
        polyline[24].x1 = 1.0;
        polyline[24].y1 = 4.0;
        polyline[24].angle = -PID;
        polyline[25].x1 = 1.0;
        polyline[25].y1 = 3.0;
        polyline[25].angle = 2.5 * PID;
    ...
    case (P_VONKOCH):
    {
        ...
        for (i = 0; i < nsides; i++)
        {
            ...
            if (i < nsides - 1)
                ...
                polyline[i].x2 = polyline[0].x1;
            if (i < nsides - 1)
                polyline[i].y2 = polyline[i + 1].y1;
            else
                polyline[i].y2 = polyline[0].y1;
        }
