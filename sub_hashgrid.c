...
/* initialise table of neighbouring cells for each hashgrid cell, depending on boundary condition */
{
    ...
    case (0): /* rectangular b.c. */
    {
        ...
        for (j = 1; j < HASHY - 1; j++)
        {
            for (i = 0; i < HASHX; i += HASHX - 1)
                hashgrid[mhash(i, j)].nneighb = 6;

            for (q = -1; q < 2; q++)
            {
                for (p = 0; p < 2; p++)
                    hashgrid[mhash(0, j)].neighbour[2 * (q + 1) + p] = mhash(p, j + q);
                for (p = -1; p < 1; p++)
                    hashgrid[mhash(HASHX - 1, j)].neighbour[2 * (q + 1) + p + 1] = mhash(HASHX - 1 + p, j + q);
            }
        }
        ...
        for (i = 1; i < HASHX - 1; i++)
        {
            for (j = 0; j < HASHY; j += HASHY - 1)
                hashgrid[mhash(i, j)].nneighb = 6;

            for (p = -1; p < 2; p++)
            {
                for (q = 0; q < 2; q++)
                    hashgrid[mhash(i, 0)].neighbour[2 * (p + 1) + q] = mhash(i + p, q);
                for (q = -1; q < 1; q++)
                    hashgrid[mhash(i, HASHY - 1)].neighbour[2 * (p + 1) + q + 1] = mhash(i + p, HASHY - 1 + q);
            }
        }
        ...
        for (j = 0; j < HASHY; j++)
            for (i = 0; i < HASHX; i += HASHX - 1)
            {
                ...
                for (p = -1; p < 2; p++)
                    ...
                    {
                        i1 = (i + p + HASHX) % HASHX;
                        if (((i == 0) && (p >= 0)) || ((i == HASHX - 1) && (p <= 0)))
                            j1 = (j + q + HASHY) % HASHY;
                        else
                            j1 = (2 * HASHY - 1 - j - q) % HASHY;
                        hashgrid[mhash(i, j)].neighbour[3 * (p + 1) + q + 1] = mhash(i1, j1);
                    }
            }
        ...
        for (i = 1; i < HASHX - 1; i++)
            for (j = 0; j < HASHY; j += HASHY - 1)
            {
                ...
                for (p = -1; p < 2; p++)
                    for (q = -1; q < 2; q++)
                    {
                        if (((j == 0) && (q >= 0)) || ((j == HASHY - 1) && (q <= 0)))
                            i1 = (i + p + HASHX) % HASHX;
                        else
                            i1 = (2 * HASHX - 1 - i - p) % HASHX;
                        j1 = (j + q + HASHY) % HASHY;
                        hashgrid[mhash(i, j)].neighbour[3 * (p + 1) + q + 1] = mhash(i1, j1);
                    }
            }
    ...
    case (4): /* genus 2 (L-shape) b.c. */
    {
        ...
        for (i = HASHX / 2; i < HASHX; i++)
            for (j = HASHY / 2; j < HASHY; j++)
                hashgrid[mhash(i, j)].nneighb = 0;
...
void update_hashgrid(t_particle *particle, t_hashgrid *hashgrid, int verbose)
{
    ...
    //         if (circleactive[k])
    {
        ...
        if (n < HASHMAX)
            hashgrid[hashcell].particles[n] = k;
        else
            printf("Too many particles in hash cell (%i, %i), try increasing HASHMAX\n", i, j);
...
/* relocate particles in case of periodic and similar boundary conditions */
{
    ...
    case (4): /* genus two (L-shaped domain) b.c. */
    {
        ...
        {
            if (x > y)
                y1 -= 0.5 * (BCYMAX - BCYMIN);
            else
                x1 -= 0.5 * (BCXMAX - BCXMIN);
            move++;
        }
    ...
    case (4): /* genus 2 (L-shaped) b.c. */
    {
        ...
        else if ((x1 >= 0.0) && (y1 < 0.0))
        {
            if (dx < -dxhalf)
                *x2 += (BCXMAX - BCXMIN);
            if (dy > 0.5 * dyhalf)
                *y2 -= 0.5 * (BCYMAX - BCYMIN);
            else if (dy < -0.5 * dyhalf)
                *y2 += 0.5 * (BCYMAX - BCYMIN);
        }
        else if ((x1 < 0.0) && (y1 >= 0.0))
        {
            if (dy < -dyhalf)
                *y2 += BCYMAX - BCYMIN;
            if (dx > 0.5 * dxhalf)
                *x2 -= 0.5 * (BCXMAX - BCXMIN);
            else if (dx < -0.5 * dxhalf)
                *x2 += 0.5 * (BCXMAX - BCXMIN);
        }
                ...
                for (k = 0; k < hashgrid[m].number; k++)
                    if ((hashgrid[m].particles[k] != j) && (particle[hashgrid[m].particles[k]].active))
                    {
                        if (n < 9 * HASHMAX)
                        {
                            ...
                            if (bc_grouped(BOUNDARY_COND) != 0)
                                wrap_relative_positions(x1, y1, &x2, &y2);
                            ...
                            printf("Not enough memory in particle.deltax, particle.deltay\n");
                    }
            }
