...
/* initialize configuration: drop at (x0,y0) for a range of particles */
{
    ...
    while (angle2 < angle1)
        angle2 += DPI;
    if (particle2 - particle1 > 1)
        dalpha = (angle2 - angle1) / ((double)(particle2 - particle1 - 1));
    else
        dalpha = 0.0;
...
/* draw zoom around target (for laser in room of mirrors) */
{
    ...
    for (i = 0; i < nparticles; i++)
    {
        ...
        else if ((vabs(x1) > 1.0) && (vabs(x2) < 1.0))
        {
            if (x2 > 0.0)
                xb = 1.0;
            else
        ...
        else if ((vabs(y1) > 1.0) && (vabs(y2) < 1.0))
        {
            if (y2 > 0.0)
                yb = 1.0;
            else
...
void animation()
{
    ...
    else if (B_DOMAIN == D_POLYLINE)
        init_polyline(polyline, circles);
    ...
    if (DRAW_BILLIARD)
        draw_billiard();
    if (PRINT_PARTICLE_NUMBER)
        print_part_number(configs, active, XMIN + 0.1, YMIN + 0.1);
    else if (PRINT_COLLISION_NUMBER)
        print_collision_number(ncollisions, XMIN + 0.1, YMIN + 0.1);
